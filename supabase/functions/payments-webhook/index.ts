...
async function handleSubscriptionCreated(data: any, env: PaddleEnv) {
  ...
  // Also mark user as premium in user_access
...
async function handleSubscriptionCanceled(data: any, env: PaddleEnv) {
  ...
  // If period has ended, revoke premium immediately
  if (sub && (!sub.current_period_end || new Date(sub.current_period_end) <= new Date())) {
    await supabase.from('user_access').update({
      is_premium: false,
    }).eq('user_id', sub.user_id);
  }
  // Otherwise, user keeps access until current_period_end (handled by frontend check)
}
